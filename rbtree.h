/*******************************************************************************
 * Name        : rbtree.h
 * Author      : Rodney Wotton and Isabel Hughes
 * Version     : 1.0
 * Date        : 11/23/2021
...
public:
     ...
     */
    explicit RedBlackTree() : root_{NULL}, size_{0} { }
...
private:
    ...
    int diameter(Node<K, V> *node) const {
        if(node == NULL){
            ...
            if(diameter(node->left) > diameter(node->right)){
                if(height(node->left)+height(node->right) + 2 > diameter(node->left)){
                    return height(node->left)+height(node->right) + 2;
                }
                else{
                    return diameter(node->left);
                }
            }
            else{
                if(height(node->left)+height(node->right) + 2 > diameter(node->right)){
                    return height(node->left)+height(node->right) + 2;
                }
                else{
                    return diameter(node->right);
                }
            }
        }
    }
    ...
    size_t sum_levels(Node<K, V> *node, size_t level) const {
        if(node != NULL){
            return sum_levels(node->left, level + 1) + sum_levels(node->right, level + 1) + level;
        }
        else{
            return 0;
        }
